"""
evaluate.py — Оценка качества выбора провода нейросетью
========================================================

КОНЦЕПЦИЯ:
  Параллельно с нейросетью запускаем несколько детерминированных алгоритмов
  выбора лучшего провода — без ML, только на основе правил из данных.
  Затем сравниваем, насколько NN-выбор близок к алгоритмическому оптимуму.

АЛГОРИТМЫ (все работают с реальным score, посчитанным через filters):
  A1. MAX_SCORE          — провод с максимальным реальным score (абсолютный оптимум)
  A2. MAX_SCORE_MIN_GCSP — при равном score берёт наименьший GCSP (как NN-ranking)
  A3. MAX_LP             — максимизирует только LEAD PREP score
  A4. MAX_LPFA           — максимизирует только LEAD PREP FA score
  A5. MAX_HV             — максимизирует только High Voltage score
  A6. MIN_GCSP           — просто самый быстрый провод (baseline — наивный выбор)
  A7. BALANCED           — максимизирует score/gcsp (эффективность на единицу времени)
  A8. WIDE_GAGE          — максимальный диапазон gage (охват операций)

МЕТРИКИ СРАВНЕНИЯ NN vs алгоритм:
  - rank_nn        : место NN-провода в рейтинге по реальному score (1 = лучший)
  - score_gap      : разрыв в score между алгоритмом и NN (0 = идеально)
  - pct_of_optimum : score NN-провода / score оптимального провода × 100%
  - top1_match     : совпадает ли NN-выбор с данным алгоритмом (0/1)
  - top5_match     : входит ли NN-выбор в топ-5 по данному алгоритму (0/1)
"""
